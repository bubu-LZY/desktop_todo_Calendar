...
### 坑 AQ：DataTemplate 里的 `x:Name` 引用，跨实例会失效 → 弹层/控件"点了没反应"

**现象**：右侧面板的时间小按钮，点它变成蓝色（选中态），但弹出的改时刻面板**打不开**。
同款按钮在别处是好的，或者"之前是好的"。

**根因**：弹层（`Popup`）的 `IsOpen` 用 `<c>{Binding #TimeToggle.IsChecked}</c>` 这种
**名字引用**，且 `Popup` 嵌在 `ToggleButton` 内部。而承载它的 `DataTemplate` 在宿主里
被**实例化了多份**（同一模板用在右侧面板 + 窄窗视图 + 各视图布局）——
`x:Name` 在 DataTemplate 跨实例解析不可靠，绑定悄悄失效，弹层就永远不打开。
这类问题**编译不报错**，运行时才暴露，而且只在"模板被复用了多份"时才发作，
单实例下完全正常，特别容易漏。

**改法**：
1. **不要用元素名字**去联动两个控件，改用**共享的 VM 属性**：
   ```xml
   <ToggleButton IsChecked="{Binding TimePopupOpen, Mode=TwoWay}" />
   <Popup IsOpen="{Binding TimePopupOpen, Mode=TwoWay}" ... />
   ```
   DataContext 绑定天然与模板实例数无关。
2. 弹层和触发按钮尽量**同级**（都放在同一个 Grid 里），`PlacementTarget` 用
   `{Binding #按钮Name}` 定位（同级下名字引用是可靠的）。
3. 需要"弹层打开期间抑制失焦提交"这类标记，也**读 VM 属性**，别再单独维护一个宿主字段。

⚠️ 通用判断：**"点了只变色/选中、但该弹出的东西没出来"**，优先怀疑
"弹层与按钮的联动断掉了"—— 要么是 `#Name` 跨实例失效，要么是 `Popup` 嵌错了位置。
先看模板有没有被复用多份，再看联动是不是靠元素名。

---

### 坑 AR：「超时 / 逾期」这类相对时间，别忘了**创建日**这个参照物

**现象**：用户"刚创建的任务马上打勾"，却提示"用时不到 1 分钟、超时 1 天完成"。

**根因**：`IsCompletedLate()` 只比「完成日 vs 到期日」，没比「创建日」。一条
**到期日之后才补建**的任务（9/23 创建、却记在 9/22），9/23 完成时"完成日 > 到期日"成立，
于是被判超时 —— 但它从未在截止前存在过，"超时"说不通。

**改法**：补一条 `CreatedDate > Date → 不算超时`（到期日之后才创建的任务谈不上"没赶上"）。

⚠️ **通用规则**：相对时间的"起算点"往往不止一个。写"超时 / 逾期 / 拖了多久"时，先问：
"这段时间的**两个端点**分别是什么？有没有哪种起点根本不该存在（比如任务还没建、截止就到了）？"
坑 AC（按任务日期还是创建时间）和坑 AR（补建不算超时）是同一个坑的两个面。

---

### 坑 AS：加速前缀（镜像）只管"下载"、不管"检查" → 检查直连被墙域名卡死

**现象**：用户填了下载加速前缀（如 `https://ghfast.top`），点「检查更新」还是一直转、卡死。
追问才知道：前缀只拼到了"下载安装包"那一步，"检查更新"是硬编码直连
`api.github.com/repos/…/releases/latest`。api.github.com 在国内被 DNS 污染 / 连接重置，
检查这步就先卡死、根本走不到下载 —— **前缀白填了**。

**根因**：写镜像前缀时默认"体积可忽略的检查走直连没关系"，但没意识到**被墙的是域名本身**，
跟体积无关。检查这一步先死，下载的加速永远没机会上场。

**改法**：`CheckAsync` 也走前缀（`前缀 + api.github.com/…`），检查与下载统一走镜像。

⚠️ **通用规则**：给"下载/上传"加镜像时，先问"这条链路上**还有哪些请求**？它们走不走镜像？"。
只要链路上有一个必须直连、而那个域名恰好被墙，整条链路就废了。加速要**整链路**生效，
不能只加速"最重的那一段"。

---

### 坑 AT：`CancellationToken` 打断不了 DNS 解析 → 超时在"连不上"时形同虚设

**现象**：代码里写了 `CancelAfter(20s)`，但用户"一直检查更新"，连超时都像不存在。

**根因**：`CancellationToken` 只能可靠打断「收发数据」阶段。域名被污染时卡在 **DNS 解析**
（底层走 Windows 系统 DNS 服务，对 token 的响应不及时甚至不响应），token 触发了取消
但请求还卡在 DNS 里出不来。

**改法**：给 `SocketsHttpHandler` 设 `ConnectTimeout`（.NET 主动给"建立连接 = DNS+TCP+TLS"整段计时，
不靠 token）。它只覆盖"建立连接"这一小段，不影响"连上后慢慢收数据" —— 大文件下载那一段仍交给
停滞看门狗（坑 AL），两者互不干扰。

⚠️ **通用规则**：`HttpClient.Timeout` / `CancelAfter` 都治不了"连都连不上"（DNS 黑洞 / SYN 一直重发）。
这类场景必须用 `ConnectTimeout`，它是**连接建立阶段**的独立计时器，语义和"响应超时"完全不同。

---

### 坑 AU：「静默下载」不等于"状态栏也别动" → 用户把后台下载误判成卡死

**现象**：点「检查更新」后状态栏一直停在"正在检查…"，过一会儿突然弹"已下载完成"。
用户以为卡死了，其实后台一直在静默下载。

**根因**：下载改成完全静默后，状态栏只在开始设了一次"正在检查…"，之后「检查 → 询问 → 下载 →
提示重启」整条流程不再更新它。下载那几十秒里状态栏纹丝不动，用户无从判断程序是否在干活。

**改法**：更新流程加 `IProgress<string>`，把「当前阶段」实时回传给状态栏：
`正在检查… → 正在后台下载更新… → 下载完成，等待确认…`。**依然没有进度条**（符合"静默"要求），
但不会让人误判卡死。

⚠️ **通用规则**：用户要"静默 / 不要进度条"，要的是**去掉进度条**，不是**去掉一切状态反馈**。
"无进度条"和"无阶段提示"是两码事。静默执行期间至少要给一个"当前在哪个阶段"的轻量回执，
否则用户必然以为程序挂了。坑 AN（VERYSILENT 档位）讲的是"装"要静默，这里讲的是"等"要可见。
...
- 加镜像/加速前缀要**整链路生效**（检查 + 下载都走），别只加速"最重的一段" —— 见坑 AS
- "连都连不上"（DNS 黑洞 / SYN 重发）的卡死用 **`ConnectTimeout`**，`CancelAfter` 治不了 DNS 阶段 —— 见坑 AT
- 用户要"静默 / 不要进度条"≠"不要任何状态反馈"：静默执行也要给**阶段回执** —— 见坑 AU
...
- 用 `x:Name` 名字引用联动 DataTemplate 里的两个控件（多实例下会失效、弹层打不开）—— 见坑 AQ
- 给"超时 / 逾期"算天数时漏掉**创建日**这个参照物（到期后补建的任务会被误判超时）—— 见坑 AR

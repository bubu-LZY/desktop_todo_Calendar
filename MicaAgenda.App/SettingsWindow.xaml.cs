...
public partial class SettingsWindow : Window
{
    ...
    {
        if (string.IsNullOrWhiteSpace(ApiTokenBox.Text)) return;
        if (Helpers.ClipboardHelper.TrySetText(ApiTokenBox.Text))
        {
            Helpers.ToastHelper.Show("Token 已复制", this);
        }
        else
        {
            ShowManualCopyFallback(ApiTokenBox.Text, "Token");
        }
    }

    /// <summary>
    /// 剪贴板被其他程序占用、自动复制失败时的兜底：弹出可手动复制（Ctrl+C）的窗口，
    /// 保证任何情况下用户都能拿到内容，而不是只看到一个 0x800401D0 报错。
    /// </summary>
    private void ShowManualCopyFallback(string text, string label)
    {
        try
        {
            var win = new Window
            {
                Title = $"复制失败 - 请手动复制{label}",
                Width = 520,
                Height = 320,
            ...
            {
                Text = "自动复制失败（剪贴板被其他程序占用）。请手动选择下方内容后按 Ctrl+C 复制：",
        ...
        {
            MessageBox.Show(this, "复制失败，内容如下：\n\n" + text, "复制失败");
        }
    }
    ...
    {
        var text = McpDocsProvider.BuildSkillDoc(McpEndpointBox.Text.Trim(), _config.ApiToken);
        if (Helpers.ClipboardHelper.TrySetText(text))
        {
            Helpers.ToastHelper.Show("Skill 文档已复制", this);
        }
        else
        {
            ShowManualCopyFallback(text, "Skill 文档");
        }
    }

    private void CopyMcpConfig_Click(object sender, RoutedEventArgs e)
    {
        var text = McpDocsProvider.BuildMcpConfigJson(McpEndpointBox.Text.Trim(), _config.ApiToken);
        if (Helpers.ClipboardHelper.TrySetText(text))
        {
        ...
        {
            ShowManualCopyFallback(text, "MCP 配置 JSON");
        }
    }
    ...
    private void OpenMindMapAgentProject_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                FileName = "https://github.com/bubu-LZY/my-mindmap-agent",
                UseShellExecute = true
            });
        }
        catch (Exception ex)
        {
            App.LogError(ex, "SettingsWindow.OpenMindMapAgentProject");
        }
    }

    private async void TestMyMindMap_Click(object sender, RoutedEventArgs e)
    {
        if (TestMyMindMapButton is null) return;
    ...
    {
        if (SyncMindMapNowButton is null) return;
        SyncMindMapNowButton.IsEnabled = false;
        SyncMindMapStatus.Text = "正在同步…";
        try
        {
            if (MindMapSyncRequested is null)
            {
        ...
        {
            if (SyncMindMapNowButton is not null) SyncMindMapNowButton.IsEnabled = true;
        }
    }
